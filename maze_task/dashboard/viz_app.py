...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                "trial version selector",
                dcc.Dropdown(
                    id="trial-version-dropdown", options=[], disabled=True, multi=True
                ),
                "trial type selector",
                dcc.Dropdown(
                    id="trial-type-dropdown", options=[], disabled=True, multi=True
                ),
...
def global_store(fileloc):
    try:
        ...
        nwbfile = io.read()
        version_ops = [
            dict(label=str(i), value=i)
            for i in set(nwbfile.trials["trial_version"].data)
        ]
        type_ops = [
            dict(label=str(i), value=i) for i in set(nwbfile.trials["trial_type"].data)
        ]
...
@app.long_callback(
    ...
    args=(
        dl.Input("submit-button-state", "n_clicks"),
...
def draw_graphs(set_progress, args):
    ...
    if versions is not None and types is not None:
        ...
        titles = [
            f"trial version {version}, trial type {type}"
            for type in types
        ...
        for trial_no in range(len(trials)):
            ...
            if (
                trials_version_data[trial_no] in versions
                and trials_type_data[trial_no] in types
            ):
                ...
                cursor_trajectory[a, b] = np.vstack(
                    [cursor_trajectory[a, b], trialized_ts[trial_no]]
                )
        c = 0

...
class MazeTaskWidget(widgets.VBox):
    def __init__(
        self, trials: TimeIntervals, timeseries: TimeSeries = None,
    ):
        super().__init__()
        self.versions = None
        self.types = None
        ...
        self.plot_button = widgets.Button(description="Click to plot")
        self.progress = widgets.FloatProgress(
            value=0.0, min=0.0, max=1.0, disabled=True
        )
    ...
    def set_children(self, fig):
        self.children = [
            widgets.HBox(children=(self.trial_type_dd, self.trial_version_dd)),
            self.plot_button,
            self.progress,
            fig,
        ]
    ...
    def trialize_time_series(self):
        ...
        for trial_no in range(len(self.trials)):
            start_id = timeseries_time_to_ind(
                self.time_series, self.trials["start_time"][trial_no]
            )
            stop_id = timeseries_time_to_ind(
                self.time_series, self.trials["stop_time"][trial_no]
            )
    ...
    def plot_trials(self):
        ...
        for trial_no in range(len(self.trials)):
            if (
                self.trials_version_data[trial_no] in self.versions
                and self.trials_type_data[trial_no] in self.types
            ):
                ...
                cursor_trajectory[a, b] = np.vstack(
                    [cursor_trajectory[a, b], self.trialized_ts[trial_no]]
                )
...
def load_maze_task_widget(node):
    default_neurodata_vis_spec[pynwb.epoch.TimeIntervals] = dict(
        {"trials table": base.render_dataframe, "task_plot": MazeTaskWidget}
    )

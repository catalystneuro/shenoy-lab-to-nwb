...
class NpxMatDataInterface(BaseDataInterface):
    def __init__(self, filename: PathType):
        ...
        self.brain_location = self.brain_location_map.get(
    ...
    def get_metadata_schema(self):
        ...
        metadata_schema["required"] = ["Behavior", "Subject", "NWBFile", "Ecephys"]
        ...
        metadata_schema["properties"]["Behavior"]["properties"] = dict(
            ...
            BehavioralTimeSeries=dict(
                type="array", items=get_schema_from_hdmf_class(TimeSeries)
            ),
        )
        ...
        metadata["Behavior"] = dict(
            Position=[
                dict(
                    ...
                    description="hand position x,y,z",
                )
            ],
            BehavioralTimeSeries=[
                dict(name="hand_speed", unit="m/s", description="hand speed in x,y,z")
            ],
        )
        return metadata
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict, **kwargs):
        ...
        obs_intervals = [
            [start_times[i], stop_times[i]] for i in range(len(start_times))
        ]
        # add behavior:
        ...
        for name, args in beh_dict.items():
            ...
            if "position" in name:
                ...
                spatial_series_list.append(
                    position_container.create_spatial_series(**args_)
                )
            else:
        ...
        for trial_no in range(self.mat_extractor._no_trials):
            col_details_dict = {
                key: args["data"][trial_no] for key, args in task_dict.items()
            }
        ...
        if len(nwbfile.electrode_groups) == 0:
            ...
            nwbfile.create_electrode_group(
                device=nwbfile.devices[args.pop("device")], **args
            )
        # add units:
        args_all = dict()
